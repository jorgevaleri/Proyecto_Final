...
<body class="body">
	...
	<main class="cuerpo">
		<div>
			<div class="title">Editar Formacion Profesional</div>
			<br>
		</div>
	...
	if(isset($_GET['id']))
	{
		...
		if(isset($_POST['modificar']))
		{
			...
			if($c==0){
				$sql="UPDATE formaciones_profesionales SET formaciones_profesionales_nombre='$formaciones_profesionales_nombre' WHERE formaciones_profesionales_id='$id'";
				$result=mysqli_query($conexion,$sql);
				
				if (mysqli_errno($conexion)==0){
					?>
					<script >	
						// alert('Se actualizo la Formacion Profesional');
					...
					?>
					<script >	
						alert('Verifique. No se actualizo la Formacion Profesional');
						location.href ='formacion_profesional.php';
					</script>	
					<?php
				}
		...
		if(!isset($_POST['modificar']) or $c!=0)
		{
			if(isset($_POST['modificar']) and $c!=0){
				...
				$sql="SELECT `formaciones_profesionales_id`, `formaciones_profesionales_nombre`, `formaciones_profesionales_eliminado` FROM `formaciones_profesionales` WHERE formaciones_profesionales_id='$id'";
				...
				else
				{
					?>
					<script >	
						alert('No existe la Formacion Profesional enviada');
						location.href ='formacion_profesional.php';
					</script>	
					<?php
				}
			...
			<form action="formacion_profesional_editar.php?tipo=<?php echo $tipo;?>&id=<?php echo $id; ?>" method="post" name="form1" id="form1">

			<table width="758" border="1" align="center">
				<tr>	
					<td height="42">	
						<p>Id: 
							<input name="formaciones_profesionales_id" type="text" id="formaciones_profesionales_id" value="<?php echo $_POST['formaciones_profesionales_id']; ?>" size="30" maxlength="60" readonly>
							<?php
							?>
						</p>
					</td>
